...
typedef boost::interprocess::allocator<std::pair<const int, BoostIpVector>,
                                       bi::managed_shared_memory::segment_manager>
    ShmemIntVecMapAllocator;

typedef bi::map<int, BoostIpVector, std::less<int>, ShmemIntVecMapAllocator> BoostIpIntVecMap;

typedef boost::interprocess::allocator<std::pair<const int, double>,
                                       bi::managed_shared_memory::segment_manager>
    ShmemIntDoubMapAllocator;

typedef bi::map<int, double, std::less<int>, ShmemIntDoubMapAllocator> BoostIpIntDoubMap;
...
public:
  PhotonSharingData(bi::managed_shared_memory & segment,
                    ...
                    int num_photon_bins,
                    int num_local_elems,
                    double total_domain_strength,
                    double local_domain_strength,
                    std::vector<double> local_domain_strengths,
                    std::vector<double> photon_bins)
    : _num_photon_bins(num_photon_bins),
      _num_local_elems(num_local_elems),
      ...
      _domain_strengths(_vec_alloc),
      _photon_bins(_vec_alloc)
  {
    ...
    for (auto & pair : photon_flux)
    {
      ...
      _photon_fluxes.insert(std::pair<int, BoostIpVector>(
          pair.first, BoostIpVector(pair.second.begin(), pair.second.end(), _vec_alloc)));
    }
